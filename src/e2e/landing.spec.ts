...
test('落地页上页头导航都不激活，但贴纸显示收录数', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '作品', exact: true })).not.toHaveAttribute('aria-current', 'page')
...
test('目录迁到 /games 且导航激活', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '作品', exact: true })).toHaveAttribute('aria-current', 'page')
})

test('从落地页进入目录', async ({ page }) => {
  await page.goto('http://localhost:4173/')
  await page.getByRole('link', { name: '进入作品目录' }).click()
...
test('空数据时统计条隐藏、精选区显示空态', async ({ page }) => {
  ...
  await expect(page.getByText('还没有收录作品。')).toBeVisible()
