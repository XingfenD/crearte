...
- 移动（`< lg`）：侧栏隐藏；结果行中的「筛选」按钮打开原生 `<dialog>` 抽屉——`paper` 底、`3px` 顶边、最高 `85vh` 可滚动、右上角关闭按钮（触摸目标 ≥ 44×44px）、底部「查看 N 款结果」按钮（点击即关闭，筛选实时生效）；遮罩 `rgba(20,20,20,.6)`（实色带透明度，非渐变）
- 抽屉交互：`showModal()` 自带 Escape 关闭与焦点圈定；关闭后焦点回到触发按钮；打开期间锁定页面滚动（`body:has(dialog[open]) { overflow: hidden }`）
- 选中态对读屏暴露：筛选行用 `aria-pressed`；抽屉触发按钮用 `aria-haspopup="dialog"` + `aria-expanded`
- 页头贴纸在 `< sm` 隐藏（导航与 logo 始终可见）

### 4.3 结果行

- 结构：`N 款游戏`（800）+ 贯通横线（`2px`）+ 排序控件（原生 `<select>`，mono、`2px` 描边、直角、右侧 `CaretDown` 下拉指示）+ 移动端「筛选」按钮（Phosphor `FunnelSimple` + 生效条件数角标）
- 排序选项沿用：最新收录 / 名称 / 时长（短到长）
- 搜索框在有输入时显示清除按钮（`aria-label="清除搜索"`，点击后清空并回焦输入框）

### 4.4 游戏卡片

- 结构：封面（16:9）→ 类型贴纸（白底 `2px` 描边、旋转 -3°、压在封面左上）→ 名称（西文 Playfair 900 / 中文系统 900）→ 简介（两行截断，`ink-soft`）→ 标签片行（时长 + 最多 2 个标签 + 溢出 `+N`，`1.5px` 描边、片内边距 `4px`，保证 `xl` 四列（约 208px 卡片）下为单行；`+N` 内含 `sr-only` 的完整标签名）
- 封面与卡片正文之间由卡片容器提供 `2px` 墨线分隔，真实封面图与兜底色块表现一致
...
- 兜底渲染：纯色块 + 首字（`coverInitial`；display 字体、白色、`hero` 比卡片大一号）；真实封面图仍优先，加载失败回落到纯色块

### 4.6 状态组件

| 状态 | 规格 |
|---|---|
| 加载中 | 3 张骨架卡（封面区与文字条用 `#EFE9DA`，`2px` 描边），透明度脉动；容器 `aria-busy="true"`，并提供「加载中」的读屏文本；网格列数与目录网格一致（`sm:2 / lg:3 / xl:4`） |
| 空结果 | `2px` 虚线直角「空展位」+ mono `NO MATCH · 0 款` + 说明 + 「重置筛选」按钮 |
| 加载失败 | 白面板 + 红标签条 `ERROR · 加载失败` + 错误信息 + mono 细节（数据路径、重试次数可选）+ 墨色「重试」按钮 |
| 404 | Playfair 巨字 `404`（`<h1>`，含 `sr-only` 的「：页面不存在」）+ 红色斜印章 `PAGE NOT FOUND`（`2.5px` 红描边、旋转 -6°）+ 说明 + 墨色「返回目录」 |
| 游戏不存在 | 虚线面板 + mono `GAME NOT FOUND` + `<h1>`「该游戏不存在或已移除。」+ 「返回目录」 |
...
- 表格：整表 `2px` 描边，表头墨底纸字，单元格 `1.5px` 底线；渲染时包在 `overflow-x: auto` 的 `.md-table-wrap` 容器内（移动端宽表可横向滚动）
- 引用：左侧 `4px` 墨条 + 纸色底
...
页头 → [文档列表 200px | 正文 max-w 640px | 本页目录 170px]。文档列表放在 `surface` 白底、`3px` 右边框的面板内（与目录侧栏同一语言），列表项 = mono 序号 + 标题，选中项黄底 `2px` 描边；正文按 §4.7；右栏目录 `3px` 左标，当前节为红色，层级缩进。移动端：列表折叠为横向滑动标签，右栏目录隐藏。
...
- 状态语义：筛选行 `aria-pressed`；抽屉触发 `aria-haspopup="dialog"` + `aria-expanded`；页头导航与文档列表当前项 `aria-current="page"`
- 标题层级：目录页 `<h1>` 为 `sr-only`「网页游戏收藏馆 · 游戏目录」；侧栏筛选有 `sr-only` 的 `<h2>筛选</h2>`，分组标签为 `<h3>`，卡片名称为 `<h2>`；详情页与文档页用可见 `<h1>`；404 与「游戏不存在」用 `<h1>`
...
@theme {
  ...
  --shadow-hard-accent-lg: 6px 6px 0 #e8552f;
}
